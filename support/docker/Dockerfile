...
RUN mvn clean install -f /opt/jboss/bpms-projects/ZipCodeServices/pom.xml \
  ...
  && chown -R jboss:jboss $BPMS_HOME/bin/.niogit $BPMS_HOME/standalone/configuration/application-roles.properties $BPMS_HOME/standalone/configuration/standalone.xml $BPMS_HOME/standalone/deployments/ZipCodeServices-1.0.war $BPMS_HOME/standalone/deployments/business-central.war/WEB-INF/classes/userinfo.properties \
